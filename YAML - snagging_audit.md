...
- **`snagging`** is treated as its own dataset so it can be expanded or linked to a mobile snagging form.
    
- **`defects_liability_period`** separates recurring issues from initial snags — helps track trends over time.
    
- **`handover_documentation`** includes practical certificates often required by UK Building Control and insurers.
    
- **`approvals_and_signoff`** formalises final acceptance (a common gap in self-builds).
    
- **`status` fields** throughout make it easy to query or filter (e.g. open snags, expired warranties).